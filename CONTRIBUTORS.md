...
- Found most CLIP models were being fed the wrong pixels, and fixed the preprocessing ([#122](https://github.com/epheterson/immich-apple-silicon/pull/122))
...
- Found HEIC decoding blocking the worker's event loop, failing thumbnails on NAS libraries ([#125](https://github.com/epheterson/immich-apple-silicon/pull/125))
...
- Found a Docker install with no running daemon crashing the accelerator instead of falling back ([#138](https://github.com/epheterson/immich-apple-silicon/pull/138))
...
Each of these is cited in a source comment explaining why some piece of this works the way it does.
...
A bug report with enough detail to reproduce is worth as much as a patch. See [CONTRIBUTING.md](CONTRIBUTING.md).
