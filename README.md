...
| **ML: CLIP** | ONNX Runtime | Native Swift: mlx (default ViT-B-32, plus the full SigLIP/SigLIP2 family) or onnxruntime (any other model, using Immich's own ONNX exports) | Same models and weights. Text embeddings match Docker exactly (cosine 1.0 in validation); image embeddings are numerically close (>0.999 cosine, measured against Immich's own ONNX export on real library previews; ~0.999 for onnxruntime-path models, resize-filter floating point). Search results are equivalent; existing indexes stay valid. |
