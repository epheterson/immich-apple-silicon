...
The full SigLIP and SigLIP2 family (17 model/resolution combinations, e.g. `ViT-SO400M-16-SigLIP2-384__webli`) also runs this faster mlx path instead of onnxruntime — 4.5x-6.1x faster on CLIP image embeddings depending on model size, measured with `scripts/native-ml-siglip-benchmark.py`. Every other zoo model still runs through onnxruntime as described above.
...
| **ML: CLIP** | ONNX Runtime | Native Swift: mlx (default ViT-B-32, plus the full SigLIP/SigLIP2 family) or onnxruntime (any other model, using Immich's own ONNX exports) | Same models and weights. Text embeddings match Docker exactly (cosine 1.0 in validation); image embeddings are numerically close (>0.999 cosine for 16 of 17 SigLIP/SigLIP2 models, ~0.98 for one — `ViT-L-16-SigLIP2-256__webli` — attributed to floating-point non-associativity across many self-attention layers, not a bug; ~0.999 for onnxruntime-path models, resize-filter floating point). Search results are equivalent; existing indexes stay valid. |
