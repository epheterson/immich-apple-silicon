...
- **Face detection missed faces Immich's own detector finds, and its confidence score was always 1.0.** The engine used Vision's landmarks request to do the detecting as well, which is a weaker detector and reports 1.000 for every face, so Immich's minimum-score setting had nothing to filter on. Detection now runs as a face-rectangles request and the landmarks pass runs on exactly those faces. Measured against Immich's own container on 24 images, at Immich's own default settings: every face 40px or larger is now found, where before it was 11 of 13, and faces between 20 and 40px went from 60% to 80%. Faces under 20px are still missed, which is Vision's floor. No false positives in either version. Immich's `minScore` is deliberately not applied to Vision's confidences, which sit on a different scale from the ONNX detector it was tuned for; applying it dropped a quarter of what we detect.
