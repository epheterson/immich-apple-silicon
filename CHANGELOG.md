...
### Changed
- **Old Immich server builds are pruned automatically.** Each extracted Immich server (`~/.immich-accelerator/server/<version>`) is roughly half a gigabyte, and until now none were ever removed, so a long-running install accumulated every version it had run. The accelerator now keeps the current build plus the most-recently-used previous one (for rollback) and prunes the rest whenever a server is resolved on start or upgrade, so the footprint self-limits.
