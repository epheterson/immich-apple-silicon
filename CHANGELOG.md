...
- **`status` said "running" whichever ML engine was running.** When the native engine cannot start, the accelerator falls back to the slower Python one. It says so once, in a line that scrolls past, and every `status` afterwards reported only "running" — so a Mac could serve every request from the fallback engine indefinitely with nothing on screen saying so. `status` now names the engine it found, and says plainly when that is not the one you configured. The engine is read from the process table rather than from the configured preference, which still says "native" while the fallback is what answers.
...
### Fixed
- **A CLIP model swap could corrupt a request that was already running.** Switching models unloaded the outgoing one by clearing its fields, but another request may still have been holding that exact instance and be mid-inference against it, which surfaced as `TypeError: 'NoneType' object is not callable` under concurrent load. A slow-loading fallback model widens the window enough to hit in practice. Unloading now only releases the shared slot and lets normal refcounting free the model once every in-flight request has finished. By [@lesurJ](https://github.com/lesurJ) ([#170](https://github.com/epheterson/immich-apple-silicon/pull/170)).
- **The menu bar app never upgraded the CLI core.** `brew outdated`, asked about a named formula, exits non-zero when that formula *is* the outdated one, and the update check treated any non-zero exit as "nothing to do". So it reported no upgrade available in exactly the case where one was, and an app kept current by Sparkle went on running whatever core it had, silently and indefinitely.
- **On a Mac carrying both Homebrew prefixes, half the accelerator looked in the wrong one.** `/usr/local` is where an x86 Homebrew installs, and a Mac migrated from Intel can have both it and `/opt/homebrew`. Four separate lookups decided which one held this install, by three different rules. The untrusted-tap warning asked the wrong `brew` and stayed silent; the native ML engine was not found and the slower Python fallback was used with nothing in the log to say why; and the upgrade check could read the other tree's version, decide it had been downgraded, and stop every service on a loop that launchd restarted forever. All four now derive from one answer: where the running code actually is.
- **A pidfile could be deleted while the service it names is healthy.** Rewriting one truncated it in place, leaving it empty on disk for an instant. `watch`, `status` and the menu bar app all read these files, the app every three seconds, and a read landing in that instant deleted the file: the service was reported stopped, `stop` became a no-op that orphaned it, and the next `start` collided on port 3003. Writes are now atomic.
- **A failed "Trust" in Settings left a red error on screen after a later one succeeded**, on every pane rather than the one it came from, and an unrelated success notice could sit beside a real failure in the same window.

### Changed
- **Face parity figures are now measured at the overlap threshold detection benchmarks use.** Two boxes counted as the same face at any overlap above zero, so a box clipping the edge of Immich's counted as a face found. At the 0.5 that COCO and WIDER FACE use, the figures in these notes and in `docs/known-differences.md` are lower and the average overlap is higher, because the near misses left the population instead of being averaged into it.
- **`scripts/ml-preflight.py` distinguishes "could not run" from "do not ship".** It fetches a face image, and doing that from inside the thread pool meant a slow link or a captive portal surfaced as failed inference and exit 1, which the gate defines as an mlx regression. The fetch now happens before the service starts and reports exit 2, which no longer resembles a crash.
...
- **A model pack that unzipped one directory deep re-downloaded 350MB on every prediction.** insightface extracts some packs to `models/<name>/<name>/`, where the loader cannot find them, so it deleted the pack and fetched it again, forever. `antelopev2` does this; `buffalo_l`, the default, does not. By [@uber1geek](https://github.com/uber1geek) ([immich-ml-metal#8](https://github.com/epheterson/immich-ml-metal/pull/8)).
- **Face detection missed faces Immich's own detector finds, and its confidence score was always 1.0.** The engine used Vision's landmarks request to do the detecting as well, which is a weaker detector and reports 1.000 for every face, so Immich's minimum-score setting had nothing to filter on. Detection now runs as a face-rectangles request and the landmarks pass runs on exactly those faces. Measured against Immich's own container on 24 images (48 reference faces, 7px to 125px), at Immich's own default settings, counting a face as found when the boxes overlap by at least 0.5 as COCO and WIDER FACE do: 23 of 48 faces are matched, up from 19. Of the faces 40px or larger, 11 of 13 are found, up from 10 of 13; faces between 20 and 40px went from 60% to 80%. Faces under 20px are all missed, which is Vision's floor. No false positives in either version: every detection overlaps a face Immich also found. Immich's `minScore` is deliberately not applied to Vision's confidences, which sit on a different scale from the ONNX detector it was tuned for; applying it dropped a quarter of what we detect.
...
- **A failed native build no longer passes silently, or leaves a broken bundle behind.** `swift build` reports compile errors on standard output, which the build script discarded along with its progress lines, so a broken build printed nothing at all. It also built straight into the destination, so a failure left either a rejected bundle or the previous one sitting where something would run it. The build is staged now and promoted only once every check passes. Contributed by [@RxChi1d](https://github.com/RxChi1d) ([#151](https://github.com/epheterson/immich-apple-silicon/pull/151)).
...
### Added
- **The whole SigLIP and SigLIP2 family now runs on the native MLX path instead of onnxruntime.** All 17 model and resolution combinations in Immich's zoo, where before only the default ViT-B-32 had a fast path. Measured on an M4: 2x-5x faster CLIP image embeddings, with the bigger models seeing the bigger win, since a fixed CPU-side resize cost dominates less when there is more GPU work to amortise it against. Embeddings were checked against Immich's own ONNX export on real library previews and agree to better than 0.999 cosine, so existing search indexes stay valid. Contributed by [@lesurJ](https://github.com/lesurJ) ([#123](https://github.com/epheterson/immich-apple-silicon/pull/123)).
  - First use of one of these models downloads its weights from the model's own Hugging Face repository, several gigabytes for the larger scales, separate from Immich's ONNX export. The menu bar shows the progress; a model that is already downloaded starts instantly.

### Fixed
- **26 of the 54 CLIP models were fed the wrong pixels.** Immich's own preprocessing config says how an image should be fitted to the model's input: most CLIP models want the short side scaled and the middle cropped, but the entire SigLIP and SigLIP2 family, plus the two `dfn5b` ViT-H-14 models, want the image squashed to a square with nothing cropped away. The accelerator always cropped. The visual tower therefore saw a centre crop of every photo while the text tower was unaffected, which quietly degrades how well a search phrase matches its picture. Found and fixed by [@lesurJ](https://github.com/lesurJ) ([#122](https://github.com/epheterson/immich-apple-silicon/pull/122)).
  - **If you use a SigLIP, SigLIP2 or dfn5b model, re-run Smart Search** (Administration → Jobs → Smart Search → All) once you are on this version. Embeddings created before it were built from the wrong crop, and a half-corrected index is worse than either.
- **The ML service could lose its port and never say so.** If something was already listening on port 3003, the service carried on running and answering nothing, because a bind failure is reported asynchronously and nothing was watching for it. Every restart after that leaked another idle process and left `ml.pid` naming one that served no traffic, so `immich-accelerator restart` would dutifully kill the wrong process and change nothing. Found on the release Mac with six such processes accumulated over four days. It now exits with the reason on the first failure.
- **A running-but-unresponsive ML service is now restarted.** The watcher treated a live PID as a working service, so anything that left the process up but not serving went unnoticed indefinitely. It now checks that the service actually answers, with a long grace period so a cold start or a multi-gigabyte first-use model download is never mistaken for a hang.
- **HEIC/RAW thumbnails could fail permanently on a NAS-backed library.** The decode shim ran the decoder synchronously, blocking the worker's event loop, and with it BullMQ's lock-renewal timer, for the whole decode. On a NAS over SMB a legitimate decode takes 20 seconds or more, which was long enough for the lock to lapse and the job to be retried forever. The decoder now runs async, so a slow decode no longer blocks anything, and decode concurrency is capped at 1 by default (`IMMICH_ACCELERATOR_HEIC_DECODE_CONCURRENCY` to raise it) since this NAS measurably slows down under concurrent reads.
- **Some videos never got a thumbnail.** ffmpeg's HEVC decoder can hard-reject a stream (real HDR10/BT.2020 phone footage) that macOS's native AVFoundation decodes fine. Confirmed not jellyfin-ffmpeg-specific: a stock Homebrew ffmpeg build fails identically. The ffmpeg wrapper now falls back to QuickLook for a single-frame thumbnail extraction that fails this way; never fires for a full video transcode.
- **`logs ml` streams as it happens** instead of sitting blank and then dumping a stale chunk. stdout is block-buffered once it is redirected to a file, under both the native and Python engines. The native engine also logs a line per request now, which the Python engine always got for free from uvicorn. Contributed by [@lesurJ](https://github.com/lesurJ) ([#121](https://github.com/epheterson/immich-apple-silicon/pull/121)).
- **Interrupted model downloads no longer strand gigabytes on disk.** The parallel fetch preallocates the whole file before writing, so an interrupted download left something reporting its full multi-gigabyte size under a name that nothing would ever clean up, and every retry added another. Stale partials are now removed before a fetch starts.
- **A model download that returns the wrong bytes is now detected.** Each chunk was accepted on its HTTP status alone, so a short or misaligned response could be written into the file, published as a complete checkpoint, and cached that way permanently. Chunk lengths are checked against what was asked for.
- **Model downloads are visible and much faster.** The native path's fetch never reported progress, so the single longest wait the service has looked like nothing was happening; it now shows in the menu bar like the ONNX one. The fetch was also effectively single-threaded despite splitting the file into eight parts, and native models were downloading ONNX weights they would never open. A 3.3GB checkpoint now lands in under a minute, and one model's cache went from 1.2GB to 33MB.
...
- **Turn the worker, ML, or dashboard off independently.** 1.8.0 made the dashboard optional; that is now one rule instead of a special case. `immich-accelerator component ml off` leaves the Mac doing thumbnails, video and metadata while an existing box keeps doing ML, which was not possible before. `component worker off` is the reverse, an ML-only compute node. `component` with no arguments lists what's on. Changes apply immediately, including to a running service, and the menu bar app's Settings window has the same three switches. Every existing install keeps all three on.
  ...
  - With ML off and no `"ml_url"` set, Immich's own Machine Learning URL applies. Its default is a Docker-internal hostname a native worker cannot resolve, so the accelerator now says so at startup rather than letting every ML job fail quietly.
...
- **`IMMICH_ACCEL_ML_THREADS` caps ML CPU use.** The ONNX models in the CLIP zoo now use every core (see below), which is right on an idle Mac and less obviously right while the worker is transcoding on the same machine. Set it to a number to cap ML without touching the worker; unset keeps the default. The cap is nearly free because the parallelism saturates early: on an M4 with ViT-B-16, capping to 2 measured 179ms against 175ms uncapped, for 8 fewer threads competing with the worker.
...
- **Settings is now a sidebar window** (General, Components, Machine Learning, Diagnostics) instead of one long column of hand-built rows. Controls line up because macOS lays them out rather than the app hand-placing them, no pane is a long scroll, and the ML self-test has a button instead of being reachable only by knowing you could click the ML row in the menu.
- **The dashboard shows Face Detection and Text (OCR) as separate bars.** They were merged into one "Face Detection · OCR" bar, which could not be accurate: faces is counted over assets that have previews and OCR over every live asset, so one bar always misreported at least one of them. Both views now show the same five queues.
...
- **Model downloads are visible.** Switching to one of the large CLIP models pulls several gigabytes on first use, and until now nothing said so: search jobs simply failed and retried while it ran. The menu bar now shows "Downloading model…" with progress, Settings explains that search jobs retry until it completes, and the fetch retries on a dropped connection instead of failing the whole model.

### Fixed
- **Large CLIP models could not be used with the native engine** (#116). Models too big for a single file store their weights separately, and the accelerator only downloaded the model definition, so the tower failed to load with no obvious error. Three models were affected: `ViT-SO400M-16-SigLIP2-384__webli`, `ViT-H-14-378-quickgelu__dfn5b`, and `XLM-Roberta-Large-ViT-H-14__frozen_laion5b_s13b_b90k`. All three were verified loading and embedding on Apple Silicon for this release; smaller models were never affected, which is why this went unnoticed in 1.7.0. Expect a multi-gigabyte download the first time Immich requests one of them. Reported by [@lesurJ](https://github.com/lesurJ).
...
- **Menu-bar cask no longer prints a deprecation warning** on every `brew update` / `brew upgrade` (#112). Switched `depends_on macos: ">= :sonoma"` to the current symbol form `depends_on macos: :sonoma` (same meaning, macOS Sonoma or newer). Reported by [@shtefko](https://github.com/shtefko).
