...
### Fixed
- **Face detection missed faces Immich's own detector finds, and its confidence score was always 1.0.** The engine used Vision's landmarks request to do the detecting as well, which is a weaker detector and reports 1.000 for every face, so Immich's minimum-score setting had nothing to filter on. Detection now runs as a face-rectangles request and the landmarks pass runs on exactly those faces. Measured against Immich's own container on 24 images: every face 40px or larger is now found, where before it was 11 of 13, and faces between 20 and 40px went from 60% to 80%. Faces under 20px are still missed, which is Vision's floor. No false positives in either version.
