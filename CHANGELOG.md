...
### Added
- **An Encoding pane in Settings, with both hardware switches.** Hardware video encoding and hardware decoding, each on by default, each with a plain description of what it costs and what it buys. The switches call the CLI (`immich-accelerator encoding hardware-video off`) rather than writing config themselves, so there is one definition of what a switch means and a test pins it to what the ffmpeg wrapper actually reads. Asked for by [@RxChi1d](https://github.com/RxChi1d) ([#155](https://github.com/epheterson/immich-apple-silicon/issues/155)).
- **Hardware decoding for thumbnails and previews.** Immich sends no `-c:v` for thumbnail jobs, so hardware decode was tied to an encoder remap that never happened and every extracted frame was decoded in software. About a quarter less CPU and a third less wall clock on those jobs. This is the one setting here that changes output: a 10-bit source yields `p010le` where Docker's software decode yields `yuv420p10le`, visually identical but not byte-identical (55.3 dB PSNR on a 4K60 HLG clip), which is why it has a switch. By [@RxChi1d](https://github.com/RxChi1d) ([#153](https://github.com/epheterson/immich-apple-silicon/pull/153)).
- **Immich's CRF setting is translated for the VideoToolbox encoders.** They ignore `-crf` and read `-q:v` on an inverted 0-100 scale, so the quality slider did nothing, and with CQ mode set to CQP it did the opposite of what it said. By [@RxChi1d](https://github.com/RxChi1d) ([#154](https://github.com/epheterson/immich-apple-silicon/pull/154)).
...
### Fixed
- **A wedged machine learning service could keep the port and never come back.** The watcher gave the engine its own session, which is exactly what takes it out of launchd's cleanup sweep, so any exit that skipped the watcher's own handler (launchd's SIGKILL after the stop grace period, a crash, `kickstart -k`) left it running. A healthy orphan gets adopted again; a wedged one held the port until someone intervened. Reported, diagnosed and fixed by [@RxChi1d](https://github.com/RxChi1d) ([#157](https://github.com/epheterson/immich-apple-silicon/pull/157)).
