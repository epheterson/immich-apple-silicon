...
### Fixes
- **Database backup files were silently 0 bytes (#24 tail)**: Immich's `DatabaseBackupService` pipes pg_dump output through `spawnDuplexStream('gzip', ['--rsyncable'])`. Apple's BSD gzip **does not support `--rsyncable`** — it's a GNU gzip extension. BSD gzip errors out immediately ("unrecognized option `--rsyncable`") and emits zero bytes. Upstream's `spawnDuplexStream` doesn't check gzip's exit code, so the pipeline resolves "cleanly" and Immich logs `Database Backup Success` on top of a 0-byte file. Every backup on a native macOS worker since the v1.4.2 shim landed was empty — including Eric's 04/14 macmini backup, verified at 0 bytes before the fix and 1,027,734,175 bytes after. **Fix**: `pg_dump_shim.js` now intercepts `gzip --rsyncable` calls too. If Homebrew's GNU gzip is installed at `/opt/homebrew/bin/gzip` (which the formula now depends on), the shim reroutes the call there — preserving upstream Immich's rsync-friendly block boundaries. Otherwise, it strips `--rsyncable` from the args and lets BSD gzip produce a valid (if not rsync-optimized) backup.
- **Homebrew formula now `depends_on "gzip"`**: fresh installs get GNU gzip automatically so `--rsyncable` works out of the box without relying on shim fallback.
- **Watchdog pattern was too broad, killing unrelated processes**: `_kill_stale_processes()` used `pgrep -f "immich|src.main"`, which matched the literal substring "immich" anywhere in a command line. That caught legitimate accelerator workers AND any unrelated process whose command line happened to contain the word — including `tart run immich-test-run-*`, `docker compose ... immich-e2e-stack`, and similar tooling. On a Mac with `immich-accelerator watch` running under launchd, every watchdog tick quietly SIGTERM'd those processes. Fix: walk `ps -axo pid,command` and filter with two precise `re.compile` patterns (`(?:^|/)node\b.*/dist/main\.js` for the worker, `(?:^|/)python[\d.]*\b.*\s-m\s+src\.main(?:\s|$)` for the ML service) that only match the canonical launch shapes from `start_service`. The first attempt at this fix used narrower `pgrep -f` patterns, but BSD pgrep's basic-regex flavor on macOS doesn't understand `\s` or unescaped `(|)` alternation — tests against Python's `re.search` went green while production was still broken. Switching both prod and tests to Python's `re` engine closed the gap.
...
### Test coverage
- `tests/test_fresh_install.py::TestKillStaleProcessesPattern` — 8 tests locking in the watchdog fix, including functional tests that actually exercise `_kill_stale_processes()` with mocked `subprocess.run` returning canned ps output and a mocked `os.kill` recording which PIDs would have been SIGTERM'd. Covers: canonical worker + ml cmdlines get killed; tracked PIDs are skipped; every shape the E2E harness produces (tart run, docker compose, socat, ssh, rsync, ad-hoc python, vim) survives; benign `python3 src/main.py` and `python3 -m src.maintenance` are left alone.
- `TestPgDumpShim::test_shim_rewrites_gzip_rsyncable_to_gnu_gzip` — real node subprocess loads the shim, calls `spawnSync('gzip', ['--rsyncable'])`, asserts exit 0 AND non-zero output bytes. A passing exit code alone (the v1.4.2-v1.4.4 failure mode) would not satisfy this test.
- `TestPgDumpShim::test_shim_gzip_without_rsyncable_is_untouched` — bare `gzip -c` must pass through unmodified.
- **`TestPgDumpShim::test_full_pipeline_produces_valid_gzipped_sql_against_db`** — real integration test Eric asked for. Spawns `/usr/lib/postgresql/14/bin/pg_dump` through the shim against the isolated Immich Postgres, pipes through `gzip --rsyncable`, asserts the resulting file is (a) non-empty, (b) valid gzip, (c) decompresses to SQL starting with `PostgreSQL database dump`. Skips gracefully if no isolated Postgres is running on 127.0.0.1:25432, so the unit suite stays portable. **Verified end-to-end on Mac mini M4** against the actual 960 MB production database before landing this fix.
