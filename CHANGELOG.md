...
- **Settings is now four tabs of native macOS forms** (General, Components, Machine Learning, Diagnostics) instead of one long column of hand-built rows. Controls line up because macOS lays them out, no tab is a long scroll, and the ML self-test has a button instead of being reachable only by knowing you could click the ML row in the menu.
